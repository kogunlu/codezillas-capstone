...
function AddNewCard()
{
    ...
    const [CardNumberFieldValidation, setCardNumberFieldValidation] = useState(true);
    const [MasterCard, setMasterCard] = useState("border-white-cyan text-white-cyan");
    ...
    function handleCardNumber(e)
    {
        ...
        const number = e.target.value ? e.target.value : setCardNumberFieldValidation('invalid:border-red-500');
        const visa = /^4[0-9]/;
    ...
    {
        e.preventDefault();
        const getMaskedDate = (value) => value.replace(/[^0-9]/g, '').replace(/^([2-9])$/g, '0$1').replace(/^(1{1})([3-9]{1})$/g, '0$1/$2').replace(/^0{1,}/g, '0').replace(/^([0-1]{1}[0-9]{1})([0-9]{1,2}).*/g, '$1/$2');
    ...
    const [Countries, setCountries] = useState([{country_name:"Loading....",country_phone_code:""}]);
    ...
    function fetchCities(city,AccessTokenA)
      ...
      const formik = useFormik({
        ...
        onSubmit: async () => {
          ...
          const [cardNumber,expDate,country,city,cvvCode,nameOnCard,zipCode,fullAdress] = [CardNumber,ExpiryDate,SelectedCountry,SelectedCity,CVV,CardHolderName,ZipCode,Adress]
    ...
    useEffect(()=>{
        ...
        .then(resul =>resul.json())
        .then(result => { fetchCities(undefined,result.auth_token);setAccessToken(result.auth_token); return result})
        .then(jsonResult => 
    ...
    return (
        <form className="w-fit mx-10 my-8 lg:mx-16 lg:my-10  xl:mx-24 xl:my-16 grid gap-y-6 md:gap-y-12"
        onSubmit={formik.handleSubmit}
        >
            <div className="grid md:grid-cols-3">
                <span className="uppercase text-2xl lg:text-5xl col-span-3 mb-3">add card details</span>
                <span className="capitalize text-la lg:text-2xl col-span-3 row-start-auto w-fit text-[#00000080]">Please make sure all of the info you enter are the same as your card registration info.</span>
            </div>
                ...
                <div className="grid gap-6 ">
                    <div className='h-20 grid content-center'>
                        ...
                        <div className="grid grid-cols-2 ">
                            <span className={`${MasterCard} border-[1px] text-center`}>MasterCard</span>
                            <span className={`${VisaCard} border-[1px] text-center`}>Visa</span>
                        </div>
                    </div>
